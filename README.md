...
Unattended smoke + `llama-bench` (SSH to the OLD PC) + EvalPlus HumanEval+ (Cursor on this VM): put `OLD_PC_SSH_USER` / `OLD_PC_SSH_PASSWORD` in `.env`, then paste the matching `evaluation/prompts/run-gpu-bench-<served_model_name>.md`. Results go to `evaluation/reports/gpu-bench-<served-model-name>.md` and that file is committed and pushed.
